...
JNI native library (FastNoiseSIMD) for **StarMade**. The game uses **Java 25** — use a **JDK 25** install for `jni.h` when compiling so the ABI matches the runtime.
...
Set **`JAVA_HOME`** to your JDK 25 root before building from an IDE or terminal (Windows: Visual Studio picks up `JAVA_HOME` for `$(JAVA_HOME)\include` in the `.vcxproj`).

**CodeBlocks:** the `.cbp` assumes a JDK at `../java/jdk-25/` (symlink or copy your JDK 25 there), or change **Project → Build options → Search directories** to your `include` and `include/linux` paths.
...
**Option A — Command Line Tools (`clang`):** from this repo, with `JAVA_HOME` pointing at JDK 25:
...
Set **`JAVA_HOME`** to JDK 25 (e.g. `C:\Program Files\Eclipse Adoptium\jdk-25...`). The `StarMadeNative.vcxproj` **IncludePath** uses `$(JAVA_HOME)\include` and `$(JAVA_HOME)\include\win32`.
